...
int main()
{
	...
	while (!input.eof())
	{
		...
		else
		{
			...
			{
				line.push_back('a');
				lowPoints.push_back(Coords(y, x));
			}
			else if (value == 'E')
			{
				line.push_back('z');
	...
	{
		int currentHeight = route[lowPoints[i].first][lowPoints[i].second];
		if (currentHeight < numSteps)
			numSteps = currentHeight;
	}
...
//Updated check from part 1 to work in reverse. We start at the highest point and can either go infinitely up or a max of 1 down
//This allows us to work backwards from any point using the same rules as part 1 and get the shortest path to the end from any spot
void ProcessSpot(std::vector<std::vector<char>> *maze, std::vector<std::vector<int>> *routeLength, std::queue<Coords> *spots)
{
