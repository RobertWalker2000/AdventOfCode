...
int SearchForAdjacentNumbers(StrArr *lines, int symbolPos);
...
int ScanMiddleLine(StrArr *lines)
{
	...
	for (int i = 0; i < lines->at(1).length(); i++)
	{
		...
		result += SearchForAdjacentNumbers(lines, i);
	}
...
int SearchForAdjacentNumbers(StrArr *lines, int symbolPos)
{
	...
	if (symbolPos > 0)
	{
		result += EvaluateAndReplaceNumber(&lines->at(0), symbolPos - 1);
		result += EvaluateAndReplaceNumber(&lines->at(1), symbolPos - 1);
		result += EvaluateAndReplaceNumber(&lines->at(2), symbolPos - 1);
	}

	result += EvaluateAndReplaceNumber(&lines->at(0), symbolPos);
	result += EvaluateAndReplaceNumber(&lines->at(2), symbolPos);
...
//First checks if the given char is a digit. If so, the full number is found, its value stored, the text string replaced with periods, and the value of the number is returned
int EvaluateAndReplaceNumber(std::string *line, int i)
{
